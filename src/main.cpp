 ...
 * Compile with:
 *
 *    c++ -I.. -O3 -std=c++11 -Wall -lsfml-system -lsfml-window -lsfml-graphics -lentityx 
...
struct Body {
  Body(const sf::Vector2f &position, const sf::Vector2f &direction, float rotationd = 
0.0)
...
public:
  explicit SpawnSystem(sf::RenderTarget &target, int count) : size(target.getSize()), 
count(count) {}
...
private:
  ...
  void collect(ex::EntityManager &entities) {
    entities.each<Body, Collideable>([this](ex::Entity entity, Body &body, Collideable 
&collideable) {
      unsigned int
  ...
  void update(ex::EntityManager &es, ex::EventManager &events, ex::TimeDelta dt) override 
{
    ...
    es.each<Particle, Body>([&vertices](ex::Entity entity, Particle &particle, Body 
&body) {
      const float r = particle.radius;
      // Spin the particles.
      sf::Transform transform;
      transform.rotate(body.rotation);
      vertices.append(sf::Vertex(body.position + 
transform.transformPoint(sf::Vector2f(-r, -r)), particle.colour));
      vertices.append(sf::Vertex(body.position + transform.transformPoint(sf::Vector2f(r, 
-r)), particle.colour));
      vertices.append(sf::Vertex(body.position + transform.transformPoint(sf::Vector2f(r, 
r)), particle.colour));
      vertices.append(sf::Vertex(body.position + 
...
class ExplosionSystem : public ex::System<ExplosionSystem>, public 
ex::Receiver<ExplosionSystem> {
public:
...
public:
  ...
  void update(ex::EntityManager &es, ex::EventManager &events, ex::TimeDelta dt) override 
{
    es.each<Body, Renderable>([this](ex::Entity entity, Body &body, Renderable 
&renderable) {
...
int main() {
  ...
  sf::RenderWindow window(sf::VideoMode::getDesktopMode(), "EntityX Example", 
sf::Style::Fullscreen);
